...
namespace basecode {
    // slab_t分配结构体，有页，free_list,指向前一个slab，指向后一个slab，缓存统计
    struct slab_t final {
        u0*             page;
        u0*             free_list;
        slab_t*         prev;
...
namespace basecode::memory::slab {
    constexpr auto slab_size = sizeof(slab_t) + alignof(slab_t); //++11 标准中，定义变量时可以用 constexpr 修饰，从而使该变量获得在编译阶段即可计算出结果的能力
    // 在aloc的slab_t链表中 删除slab
    ...
    //free alloc mem 大小的内存
    static u0 free(alloc_t* alloc, u0* mem, u32& freed_size) {
        auto sc = &alloc->subclass.slab;

        auto curr = sc->head;
        while (curr) {
            if (mem >= curr->page &&  mem <= ((u8*) curr->page + sc->page_size - slab_size)) {
                break;
            }
            curr = curr->next;
        }
        ...
        *((u0**) mem) = slab->free_list;
        slab->free_list = mem; //将mem 添加到slab 的free_list中，buf_count--
        --slab->buf_count;

        if (slab->buf_count == 0) { // 当buf_count变成0，删除alloc 中的slab
            remove(alloc, slab);
