...
namespace basecode::path {
    ...
    constexpr s8 path_sep = '/';
#endif
    // 将给定的path 标准化，按照定义的struct生成一些信息存到struct中
...
#ifdef _WIN32
            if (isalpha(ch)) {
                if (i + 1 < path.str.length && path.str[i + 1] == ':') {
                    path_mark_t mark{};
